const signalFilter = (signal, weights, { output = null }) => {
  const filtered =
    output || new Float64Array(signal.length - weights.length + 1);
...
const getMaximaIndices = (signal, { buffer = null }) => {
  ...
  d1.forEach((value, index, array) => {
    ...
    if (d1[index] > 0 && d1[index + 1] < 0) {
      maximas.push(
        Math.abs(d1[index]) < Math.abs(d1[index + 1]) ? index : index + 1
      );
    }
  });
...
export class WebTuner {
  async init() {
    try {
      ...
      this.data.smoothed = new Float64Array(
  ...
  detectNotes() {
    ...
    signalFilter(signal, [0.1, 0.2, 0.4, 0.2, 0.1], { output: smoothed });
    const maxSignal = Math.max(...signal);

    const maximaIndices = getMaximaIndices(smoothed, { buffer: d1 }).map(
      x => x + 2
    );
    ...
    const detectedNotes = maximaIndices
      .map(x =>
        Object.assign(
          { x: x, freq: x * interval, decibel: signal[x] },
          getNote(x * interval)
        )
      )
...
export const summarize = detectedNotes => {
  ...
  const result = Object.entries(grouped)
    .map(([key, value]) => {
      ...
      const weightedResidueSum = value
        .map(x => x.residue * Math.pow(10, (x.decibel - maxDecibel) / 10))
        .reduce((r, x) => r + x, 0);
      const weightSum = value
        .map(x => Math.pow(10, (x.decibel - maxDecibel) / 10))
        .reduce((r, x) => r + x, 0);
      return {
        noteName: key,
